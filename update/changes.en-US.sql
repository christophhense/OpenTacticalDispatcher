...
UPDATE `hints` SET `hint` = 'SMTP server for sending emails. Protocol, encryption and port information are optional. With \"smtps://\" or \"tls://\" the connection is only established encrypted, with \"smtp://\" or without specification an encrypted connection is only established if the server offers it. Example: smtps://mail.example.com:25. If the Internet connection is potentially unstable, it is advisable to leave the delivery to a local smpt server, e.g. Postfix (Unix / Linux) or h-mail server (Windows).' WHERE `tag` = '_api_email_smtp_host';

...
UPDATE `hints` SET `hint` = 'SMTP-Server für E-Mailversand. Angaben zu Protokoll, Verschlüsselung und Port sind optional. Bei \"smtps://\" oder \"tls://\" wird die Verbindung nur verschlüsselte aufgebaut, bei \"smtp://\" oder ohne Angabe wird eine verschlüsselte Verbindung nur aufgebaut, wenn der Server dies anbietet. Bsp.: smtps://mail.example.com:25. Bei potentiell instabiler Internetverbindung bietet es sich an, die Zustellung einem lokalen smpt-Server zu überlassen, z.B. Postfix (Unix / Linux) oder h-Mailserver (Windows).' WHERE `tag` = '_api_email_smtp_host';
